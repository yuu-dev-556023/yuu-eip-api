...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Eip") == null)
        {
    ...
    private async Task CreateApplicationsAsync()
    {
        var commonScopes = new List<string> {
            ...
            OpenIddictConstants.Permissions.Prefixes.Resource + "https://localhost:44319/mcp",
            OpenIddictConstants.Permissions.Prefixes.Resource + "https://yuu-eip.yuu-is87.com/mcp",
            "Eip"
        };
        ...
        var consoleAndAngularClientId = configurationSection["Eip_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["Eip_App:RootUrl"]?.TrimEnd('/');
        ...
        /* Swagger Client */
        var swaggerClientId = configurationSection["Eip_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Eip_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        /* VS Code github copilit */
        var vscodeClientId = "VS_Code";
        if (!vscodeClientId.IsNullOrWhiteSpace())
        {
            await CreateApplicationAsync(
                name: vscodeClientId!,
                type: OpenIddictConstants.ClientTypes.Public,
        ...
        List<string>? permissions = null)
    {
        ...
        foreach (var scope in scopes)
        {
            ...
            else if (scope.Contains(OpenIddictConstants.Permissions.Prefixes.Resource))
            {
                application.Permissions.Add(scope);
            }
            else
